...
* EVT - represents the current distribution of the terrestrial ecological systems classification, developed by NatureServe for the western hemisphere, through 2016
* EVC - represents the vertically projected percent cover of the live canopy layer for a 30-m cell
* EVH - represents the average height of the dominant vegetation for a 30-m cell
...
* Not surprisingly, shrub cover is dominant in this subregion with most shrub height(s) concentrated between .5 m and .9 m
...
The data from chart below is displayed spatially on the map (directly) below the chart. The colors on the graph correspond to the EVC classifications on the landscape and the map is a visual, spatial representation of the same information.

For example, the majority of this subregion is classified as "shrub," which aligns with the information conveyed on the chart (above) where shrub cover comprises approximately 80% of the vegetation here.
...
The data from chart below is displayed spatially on the map (directly) below the chart. The colors on the graph correspond to the EVH height classifications on the landscape showing how majority of the the vegetation height is concentrated in the .25-.9 m height category.
